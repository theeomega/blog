...
We will use a conditional error like (1/0), which will trigger an error and help us distinguish between true and false. The idea is to check each character of the flag with our input using substr() and cause an error based on the comparison.
...
Yes. It works. Now lets create our basic payload for final testing.
